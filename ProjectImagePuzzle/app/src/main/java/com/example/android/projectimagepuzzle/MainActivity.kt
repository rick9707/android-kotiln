...
import com.example.android.projectimagepuzzle.util.MyGlideEngine
...
//        this.buttonCamera.setOnClickListener { this.chooseMedia(true) }
    ...
    private fun chooseMedia(isCameraType: Boolean = false)
    {
        ...
        {
            this.onPermissionsGranted(REQUEST_CODE, arrayListOf())
        }
        else
        {
            EasyPermissions.requestPermissions(this, "Permissions for next step.", REQUEST_CODE, *this.permissions)
        }
    }
    ...
    override fun onPermissionsGranted(requestCode: Int, perms: MutableList<String>)
    {
        if (requestCode == REQUEST_CODE)
        {
            Matisse.from(this)
                    ...
                    .capture(true)
                    .captureStrategy(CaptureStrategy(true, PROVIDER_AUTHORITY))
                    .countable(true)
                    .maxSelectable(1)
                    .restrictOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED)
                    .thumbnailScale(0.75f)
                    .imageEngine(MyGlideEngine())
                    .forResult(REQUEST_RESULT)
        }
    }
